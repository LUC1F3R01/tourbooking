...
def packageCancel(request,id):
	...
	if request.method =="POST":
		...
		amount = order.amount
		sender_email = "pansuriyadenish1@gmail.com"  # Enter your address
...
def handleSignUp(request):
	...
	if request.method == "POST":
		...
		sender_email = "pansuriyadenish1@gmail.com"  # Enter your address
		receiver_email = (str(email),)  # Enter receiver address
		message = f"\nSubject: Hi {fname}\t {lname} \n \t Thank you for creating account of rising adventure. You have successfully created acount\n please update your profile details in Profile section \n regards team rising adventure"
		subject = f"About your sucessfully account creation."
		send_mail(subject, message, sender_email, receiver_email, fail_silently=True)
...
def customer_reports(request):
    ...
    return render(request, 'customer_report.html',{'customers' : customers, 'currentdate': currentdate})

@pdf_decorator(pdfname='Booking.pdf')
def booking_reports(request):
    currentdate = datetime.datetime.now
    bookings = Package_Booking.objects.exclude(status=3)
    return render(request, 'booking-report.html',{'bookings' : bookings, 'currentdate': currentdate})

# @pdf_decorator(pdfname='Order.pdf')
# def order_reports(request):
#     currentdate = datetime.datetime.customer_reports
...
def cancel_reports(request):
    currentdate = datetime.datetime.now
    cancels = PackageCancel.objects.all()
    return render(request, 'cancel-report.html',{'cancels' : cancels, 'currentdate': currentdate})
	...
	if request.method=="POST":
		...
		try:
			...
			if password == password2:
				...
				return redirect('./password_resert_done.html')
		except:
...
def send_otp(request):
	sender_email = 'pansuriyadenish1@gmail.com'
	email=request.POST.get("email")
...
def invoice_action(request):
	...
	return render(request,"package/invoice.html" , context)